...
export async function POST(req: NextRequest) {
  ...
  const { nomeConvidado, telefone } = (body ?? {}) as {
    nomeConvidado?: string;
    telefone?: string;
  };

  const nomeTrim = nomeConvidado?.trim() ?? "";
  const telefoneTrim = telefone?.trim() ?? "";

  if (nomeTrim.length < 2) {
    return NextResponse.json(
      { ok: false, error: "Indica o nome do(a) convidado(a)." },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    const sql = getSql();
    await sql`
      INSERT INTO rsvps (nome_convidado, telefone)
